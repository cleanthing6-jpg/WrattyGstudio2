...
TABLE = {
    "afrobeats": "afrobeats", "afrobeat": "afrobeats", "afropop": "afrobeats",
    "afroswing": "afrobeats", "amapiano": "afrobeats", "dancehall": "afrobeats",
    "afro": "afrobeats",
    "rap": "rap", "hiphop": "rap", "trap": "rap", "drill": "rap", "grime": "rap",
    "rnb": "rnb", "randb": "rnb", "rb": "rnb", "soul": "rnb",
    "pop": "pop", "dance": "pop", "edm": "pop", "house": "pop",
    "neutral": "neutral", "auto": "neutral", "none": "neutral", "default": "neutral",
}
def slug_of(t):
    return TABLE.get(re.sub(r"[^a-z0-9]", "", t.lower()))

entries = list(re.finditer(r"\{[^{}]*\}", body, re.S))
if not entries:
    sys.exit("ABORT: no { value, label } entries inside STYLES")

newbody, rows = body, []
for e in entries:
    etext = e.group(0)
    vl = re.search(r'value:\s*"([^"]*)"', etext)
    ll = re.search(r'label:\s*"([^"]*)"', etext)
    if not vl or not ll:
        sys.exit("ABORT: entry missing value/label -> " + etext[:90])
    old, label = vl.group(1), ll.group(1)
    new = slug_of(label) or slug_of(old)
    if not new or new not in slugs:
        sys.exit("ABORT: cannot map %r (value %r) to an engine preset" % (label, old))
...
if tsx.count(OLD) != 1:
    sys.exit("ABORT: /api/mix body matched %d times (need 1)" % tsx.count(OLD))
tsx = tsx.replace(OLD, NEW, 1)

COMP.write_text(tsx, encoding="utf-8")
for label, old, new in rows:
    print("  %-16s %-12s -> %s" % (label, old, new))
